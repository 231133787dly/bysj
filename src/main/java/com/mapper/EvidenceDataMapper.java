...
public interface EvidenceDataMapper {
    ...
    @Select("select * from evidence_data_18h where device_serial=#{deviceSerial} and datetime >= #{startTime} and datetime < #{endTime}")
    @Results({
            @Result(id=true,property="id",column="id",javaType=Integer.class),
            @Result(property="estimatedBeats",column="estimated_beats",javaType=String.class),
            @Result(property="actualBeats",column="actual_beats",javaType=String.class),
            @Result(property="beatsRate",column="beats_rate",javaType=Integer.class),
            @Result(property="maxBeatsLackTime",column="max_beats_lack_time",javaType=String.class),
    ...
    @Select("select * from evidence_data_14h where device_serial=#{deviceSerial} and datetime >= #{startTime} and datetime < #{endTime}")
    @Results({
            @Result(id=true,property="id",column="id",javaType=Integer.class),
            @Result(property="estimatedBeats",column="estimated_beats",javaType=String.class),
            @Result(property="actualBeats",column="actual_beats",javaType=String.class),
    ...
    @Select("select * from evidence_data_12h where device_serial=#{deviceSerial} and datetime >= #{startTime} and datetime < #{endTime}")
    @Results({
            @Result(id=true,property="id",column="id",javaType=Integer.class),
            @Result(property="estimatedBeats",column="estimated_beats",javaType=String.class),
            @Result(property="actualBeats",column="actual_beats",javaType=String.class),
    ...
    @Select("select * from evidence_data_11h where device_serial=#{deviceSerial} and datetime >= #{startTime} and datetime < #{endTime}")
    @Results({
            @Result(id=true,property="id",column="id",javaType=Integer.class),
            @Result(property="estimatedBeats",column="estimated_beats",javaType=String.class),
            @Result(property="actualBeats",column="actual_beats",javaType=String.class),
